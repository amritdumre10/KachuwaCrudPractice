                    ...
                    else
                    {
                        switch (customAtt.fillBy)
                        {
                            ...
                            case AutoFillProperty.CurrentUserId:
                                var userId = ContextResolver.Context.User.Claims.SingleOrDefault(c => c.Type == "IdUid");
                                customAtt.DefaultValue = userId == null ? 0 : Convert.ToInt32(userId.Value.ToString());
                                break;
